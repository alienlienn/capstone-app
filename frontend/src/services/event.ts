...
export async function fetchEventsBySchoolId(schoolId: number): Promise<CalendarEvent[]> {
  try {
    const response = await fetch(`${ENV.API_BASE_URL}/event/get_school_events/${schoolId}`);
    if (!response.ok) {
      console.error("Failed to fetch events:", response.statusText);
      return [];
    }
      ...
      data.events.forEach((event: any) => {
        if (event.start_datetime) {
          events.push({
            ...
            startTime:
              event.start_time ||
              (event.start_datetime.split("T")[1]?.substring(0, 5) === "00:00"
                ? null
                : event.start_datetime.split("T")[1]?.substring(0, 5)),
            endTime:
              event.end_time ||
              (event.end_datetime
                ? event.end_datetime.split("T")[1]?.substring(0, 5) === "00:00"
                  ? null
                  : event.end_datetime.split("T")[1]?.substring(0, 5)
                : null),
            createdBy: event.created_by,
          });
        }
